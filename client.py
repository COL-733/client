...
class Client:
    def __init__(self, server_host: str, server_port: int):
        ...
        self.cv = Condition()
        self.list_res: list[set[VersionedValue]] = []
    ...
    def reconcile_and_send(self):
        """Reconcile versioned values and send appropriate request"""
        with self.cv:
            if len(self.list_res) == 0:
                return True
            elif len(self.list_res) == 1 and self.current_request_type == MessageType.GET:
                return True
            else:
                all_values = set()
                ...
                if self.current_request_type == MessageType.GET:
                    message = Message(
                        self.current_msg_id,
                        MessageType.GET,
                        self.current_source,
    ...
    def handle_switch(self, msg: Message):
        with self.cv:
            if msg.msg_type in {MessageType.GET, MessageType.PUT}:
                self.list_res.append(msg.kwargs.get("context", set()))
                self.cv.notify()
                return 0
            elif msg.msg_type in {MessageType.GET_RES, MessageType.PUT_ACK}:
                self.cv.notify()
                return msg.kwargs.get("res")
    ...
    def send_get_request(self, msg_id: str, source: str, destination: str, key: str):
        with self.cv:
            ...
            try:
                ...
                self.cv.wait()
                if self.list_res:
                    return self.reconcile_and_send()
                return True
            except Exception as e:
                logging.error(f"Failed to send GET message: {e}")
                raise

    def send_put_request(self, msg_id: str, source: str, destination: str, key: str, value: str):
        with self.cv:
            self.current_msg_id = msg_id
            self.current_source = source
            self.current_dest = destination
            self.current_key = key
            self.current_request_type = MessageType.PUT
            self.list_res = []

            message = Message(msg_id, MessageType.PUT, source, destination, key=key, value=value)
            try:
                self.socket.send(message.serialize())
